...
',
          ...
          bless( {
                   ...
                   'type' => Compiler::Lexer::TokenType::T_HereDocument,
                   'stype' => Compiler::Lexer::SyntaxType::T_Value,
                   'data' => '    op-entry { printf("op-entry <%s>\\n", copyinstr(arg0)) }
',
                   'name' => 'HereDocument'
                 }, 'Compiler::Lexer::Token' ),
          bless( {
...
'
          ...
          bless( {
                   ...
                   'line' => 144,
                   'name' => 'HereDocument',
                   'data' => '    loading-file { printf("loading-file <%s>\\n", copyinstr(arg0)) }
    loaded-file  { printf("loaded-file <%s>\\n", copyinstr(arg0)) }
'
                 }, 'Compiler::Lexer::Token' ),
          bless( {
          ...
          bless( {
                   'name' => 'RegExp',
                   'data' => 'loading-file <strict\\.pm>.*loading-file <HTTP/Tiny\\.pm>.*loading-file <run/dtrace\\.pl>',
                   'line' => 149,
                   'type' => Compiler::Lexer::TokenType::T_RegExp,
                   'has_warnings' => 0,
                   'kind' => Compiler::Lexer::Kind::T_Term,
          ...
          bless( {
                   ...
                   'stype' => Compiler::Lexer::SyntaxType::T_Value,
                   'name' => 'RegExp',
                   'data' => 'loaded-file <strict\\.pm>.*loaded-file <HTTP/Tiny\\.pm>.*loaded-file <run/dtrace\\.pl>'
                 }, 'Compiler::Lexer::Token' ),
          bless( {
                   'line' => 150,
                   'has_warnings' => 0,
          ...
          bless( {
                   'name' => 'RawString',
                   'data' => 'loading-file, loaded-file probes',
                   'type' => Compiler::Lexer::TokenType::T_RawString,
                   'has_warnings' => 0,
                   'kind' => Compiler::Lexer::Kind::T_Term,
                   'stype' => Compiler::Lexer::SyntaxType::T_Value,
...
<- Your::inner at - line 4!/ , \'traced multiple function calls\' , ) ;'
    ...
    \' , qr/foo during INIT; baz during END;/ , \'make sure sub-entry and phase-change interact well\' , ) ;',
          ...
          },
          {
            ...
            'block_id' => 0,
            'token_num' => 23,
            'src' => ' dtrace_like ( q{    my $tmp = "foo";
    $tmp =~ s/f/b/;
    chop $tmp;
} , q{    op-entry { printf("op-entry <%s>\\n", copyinstr(arg0)) }
} , [ qr/op-entry <subst>/ , qr/op-entry <schop>/ , ] , \'basic op probe\' , ) ;',
            'indent' => 0,
            'start_line' => 120
          },
          {
            ...
            'end_line' => 153,
            'has_warnings' => 1,
            'src' => ' dtrace_like ( q{    BEGIN {@INC = \'../lib\'}
    use strict;
    require HTTP::Tiny;
    do "run/dtrace.pl";
} , q{    loading-file { printf("loading-file <%s>\\n", copyinstr(arg0)) }
    loaded-file  { printf("loaded-file <%s>\\n", copyinstr(arg0)) }
} , [ qr{loading-file <strict\\.pm>.*loading-file <HTTP/Tiny\\.pm>.*loading-file <run/dtrace\\.pl>}s , qr{loaded-file <strict\\.pm>.*loaded-file <HTTP/Tiny\\.pm>.*loaded-file <run/dtrace\\.pl>}s , ] , \'loading-file, loaded-file probes\' , ) ;',
            'indent' => 0,
            'start_line' => 135
          },
          {
            'indent' => 0,
            'src' => ' sub dtrace_like { my $perl = shift ; my $probes = shift ; my $expected = shift ; my $name = shift ; my ( $reader , $writer ) ; my $pid = open2 ( $reader , $writer , $dtrace , \'-q\' , \'-n\' , \'BEGIN { trace("ready!\\n") }\' , \'-n\' , $probes , \'-c\' , $Perl , ) ; chomp ( my $throwaway = < $reader > ) ; $throwaway eq "ready!" or die "Unexpected \'ready!\' result from DTrace: $throwaway" ; print $writer $perl ; close $writer ; local $/ ; my $result = < $reader > ; waitpid ( $pid , 0 ) ; my $child_exit_status = $? >> 8 ; die "Unexpected error from DTrace: $result" if $child_exit_status != 0 ; if ( ref ( $expected ) eq \'ARRAY\' ) { like ( $result , $_ , $name ) for @$expected ; } else { like ( $result , $expected , $name ) ; } }',
            'start_line' => 155,
            'has_warnings' => 1,
            'end_line' => 196,
            'token_num' => 147,
          ...
          },
          {
            ...
            'token_num' => 22,
            'block_id' => 2,
            'src' => ' if ( ref ( $expected ) eq \'ARRAY\' ) { like ( $result , $_ , $name ) for @$expected ; }',
            'indent' => 1,
            'start_line' => 190
          },
          {
            'start_line' => 191,
            'src' => ' like ( $result , $_ , $name ) for @$expected ;',
            'indent' => 2,
            'token_num' => 11,
            'block_id' => 3,
            'has_warnings' => 1,
          ...
          },
          {
            ...
            'token_num' => 12,
            'block_id' => 2,
            'src' => ' else { like ( $result , $expected , $name ) ; }',
            'indent' => 1,
            'start_line' => 193
          },
          {
