...
subtest 'get_groups_by_syntax_level' => sub {
    ...
    is_deeply($stmts, [
          ...
          },
          {
            ...
            'has_warnings' => 1,
            'end_line' => 66,
            'src' => ' sub fork_and_retrieve { my $which = shift ; pipe my ( $r , $w ) or die "pipe: $!\\n" ; my $pid = fork ; defined $pid or die "fork: $!\\n" ; if ( $pid ) { close $w ; $_ = < $r > ; chomp ; die "Garbled output \'$_\'" unless my ( $first , $second ) =/^(\\d+),(\\d+)\\z/ ; cmp_ok ( $first , \'>=\' , 1 , "Parent of $which grandchild" ) ; cmp_ok ( $second , \'>=\' , 1 , "New parent of orphaned $which grandchild" ) ; SKIP : { skip ( "Orphan processes are not reparented on QNX" , 1 ) if $^O eq \'nto\' ; isnt ( $first , $second , "Orphaned $which grandchild got a new parent" ) ; } return $second ; } else { $: : NO_ENDING = 1 ; close $r ; my $pid2 = fork ; defined $pid2 or die "fork: $!\\n" ; if ( $pid2 ) { close $w ; sleep 1 ; } else { my $ppid1 = getppid ( ) ; sleep 2 ; my $ppid2 = getppid ( ) ; print $w "$ppid1,$ppid2\\n" ; } exit 0 ; } }',
            'start_line' => 23,
            'indent' => 0,
            'block_id' => 0
          },
          ...
          },
          {
            ...
            'has_warnings' => 1,
            'end_line' => 44,
            'src' => ' if ( $pid ) { close $w ; $_ = < $r > ; chomp ; die "Garbled output \'$_\'" unless my ( $first , $second ) =/^(\\d+),(\\d+)\\z/ ; cmp_ok ( $first , \'>=\' , 1 , "Parent of $which grandchild" ) ; cmp_ok ( $second , \'>=\' , 1 , "New parent of orphaned $which grandchild" ) ; SKIP : { skip ( "Orphan processes are not reparented on QNX" , 1 ) if $^O eq \'nto\' ; isnt ( $first , $second , "Orphaned $which grandchild got a new parent" ) ; } return $second ; }',
            'start_line' => 28,
            'indent' => 1,
            'block_id' => 3
          },
          ...
          },
          {
            ...
            'has_warnings' => 1,
            'end_line' => 34,
            'src' => ' die "Garbled output \'$_\'" unless my ( $first , $second ) =/^(\\d+),(\\d+)\\z/ ;',
            'start_line' => 33,
            'indent' => 2,
            'block_id' => 4
          },
          ...
          },
          {
            ...
            'has_warnings' => 1,
            'end_line' => 36,
            'src' => ' cmp_ok ( $second , \'>=\' , 1 , "New parent of orphaned $which grandchild" ) ;',
            'start_line' => 36,
            'indent' => 2,
            'block_id' => 4
          },
          ...
          },
          {
            ...
            'has_warnings' => 1,
            'end_line' => 65,
            'src' => ' else { $: : NO_ENDING = 1 ; close $r ; my $pid2 = fork ; defined $pid2 or die "fork: $!\\n" ; if ( $pid2 ) { close $w ; sleep 1 ; } else { my $ppid1 = getppid ( ) ; sleep 2 ; my $ppid2 = getppid ( ) ; print $w "$ppid1,$ppid2\\n" ; } exit 0 ; }',
            'start_line' => 45,
            'indent' => 1,
            'block_id' => 3
          },
          ...
          },
          {
            ...
            'has_warnings' => 1,
            'end_line' => 63,
            'src' => ' else { my $ppid1 = getppid ( ) ; sleep 2 ; my $ppid2 = getppid ( ) ; print $w "$ppid1,$ppid2\\n" ; }',
            'start_line' => 56,
            'indent' => 2,
            'block_id' => 6
          },
