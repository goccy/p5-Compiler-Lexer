...
subtest 'tokenize' => sub {
    ...
    is_deeply($tokens, [
          ...
          bless( {
                   'data' => 'ungetc of $j returns itself',
                   'type' => Compiler::Lexer::TokenType::T_String,
                   'kind' => Compiler::Lexer::Kind::T_Term,
                   'has_warnings' => 0,
                   'stype' => Compiler::Lexer::SyntaxType::T_Value,
...
subtest 'get_groups_by_syntax_level' => sub {
    ...
    is_deeply($stmts, [
          ...
          },
          {
            ...
            'indent' => 0,
            'token_num' => 8,
            'src' => ' my $crlf = uni_to_native ( "\\015\\012" ) ;'
          },
          {
            'start_line' => 15,
            'indent' => 0,
            'block_id' => 0,
            'src' => ' my $crcr = uni_to_native ( "\\x0d\\x0d" ) ;',
            'token_num' => 8,
            'end_line' => 15,
            'has_warnings' => 1
          },
          {
            ...
            'start_line' => 17,
            'block_id' => 0,
            'src' => ' my $ungetc_count = 8200 ;',
            'token_num' => 5,
            'end_line' => 17,
            'has_warnings' => 0
          },
          {
            'src' => ' { plan ( tests => 16 + 2 * $ungetc_count ) ; ok ( open ( FOO , ">:crlf" , $file ) ) ; ok ( print FOO \'a\' . ( ( ( \'a\' x 14 ) . qq{\\n} ) x 2000 ) || close ( FOO ) ) ; ok ( open ( FOO , "<:crlf" , $file ) ) ; my $text ; { local $/ ; $text = < FOO > } is ( count_chars ( $text , $crlf ) , 0 ) ; is ( count_chars ( $text , "\\n" ) , 2000 ) ; binmode ( FOO ) ; seek ( FOO , 0 , 0 ) ; { local $/ ; $text = < FOO > } is ( count_chars ( $text , $crlf ) , 2000 ) ; SKIP : { skip_if_miniperl ( "miniperl can\'t rely on loading PerlIO::scalar" , 2 * $ungetc_count + 1 ) ; skip ( "no PerlIO::scalar" , 2 * $ungetc_count + 1 ) unless $Config { extensions } =~ m!\\bPerlIO/scalar\\b! ; require PerlIO::scalar ; my $fcontents = join "" , map { "$_$crlf" } "a" .. "zzz" ; open my $fh , "<:crlf" , \\ $fcontents ; local $/ = "xxx" ; local $_ = < $fh > ; my $pos = tell $fh ; seek $fh , $pos , 0 ; $/ = "\\n" ; $s = < $fh > . < $fh > ; is ( $s , "\\nxxy\\n" ) ; for my $i ( 0 .. $ungetc_count - 1 ) { my $j = $i % 256 ; is ( $fh-> ungetc ( $j ) , $j , "ungetc of $j returns itself" ) ; } for ( my $i = $ungetc_count - 1 ; $i >= 0 ; $i -- ) { my $j = $i % 256 ; is ( ord ( $fh-> getc ( ) ) , $j , "getc gets back $j" ) ; } } ok ( close ( FOO ) ) ; for my $utf8 ( \'\' , \':utf8\' ) { for my $binmode ( 1 .. 2 ) { open ( FOO , ">$file" ) ; binmode ( FOO , "$utf8:crlf" ) for 1 .. $binmode ; print FOO "Hello\\n" ; close FOO ; open ( FOO , "<$file" ) ; binmode ( FOO ) ; my $foo = scalar < FOO > ; close FOO ; print join ( " " , "#" , map { sprintf ( "%02x" , $_ ) } unpack ( "C*" , $foo ) ) , "\\n" ; like ( $foo , qr/$crlf$/ ) ; unlike ( $foo , qr/$crcr/ ) ; } } }',
            'token_num' => 443,
            'indent' => 0,
            'start_line' => 19,
            'block_id' => 0,
          ...
          {
            'src' => ' plan ( tests => 16 + 2 * $ungetc_count ) ;',
            'token_num' => 11,
            'indent' => 1,
            'start_line' => 20,
            'block_id' => 2,
          ...
          {
            'src' => ' is ( count_chars ( $text , $crlf ) , 0 ) ;',
            'token_num' => 12,
            'indent' => 1,
            'start_line' => 27,
            'block_id' => 2,
          ...
          },
          {
            'has_warnings' => 1,
            'end_line' => 33,
            'src' => ' is ( count_chars ( $text , $crlf ) , 2000 ) ;',
            'token_num' => 12,
            'start_line' => 33,
            'block_id' => 2,
            'indent' => 1
          },
          {
            'has_warnings' => 1,
            'end_line' => 38,
            'src' => ' skip_if_miniperl ( "miniperl can\'t rely on loading PerlIO::scalar" , 2 * $ungetc_count + 1 ) ;',
            'token_num' => 11,
            'block_id' => 5,
            'start_line' => 37,
            'indent' => 2
          },
          {
            ...
            'indent' => 2,
            'token_num' => 21,
            'src' => ' skip ( "no PerlIO::scalar" , 2 * $ungetc_count + 1 ) unless $Config { extensions } =~ m!\\bPerlIO/scalar\\b! ;'
          },
          {
            'end_line' => 41,
            'has_warnings' => 1,
          ...
          {
            'src' => ' my $fcontents = join "" , map { "$_$crlf" } "a" .. "zzz" ;',
            'token_num' => 14,
            'start_line' => 42,
            'block_id' => 5,
            'indent' => 2,
          ...
          },
          {
            ...
            'indent' => 2,
            'block_id' => 5,
            'src' => ' for my $i ( 0 .. $ungetc_count - 1 ) { my $j = $i % 256 ; is ( $fh-> ungetc ( $j ) , $j , "ungetc of $j returns itself" ) ; }',
            'token_num' => 33,
            'end_line' => 55,
            'has_warnings' => 1
          },
          ...
          {
            'token_num' => 14,
            'src' => ' is ( $fh-> ungetc ( $j ) , $j , "ungetc of $j returns itself" ) ;',
            'indent' => 3,
            'start_line' => 54,
            'block_id' => 6,
            'has_warnings' => 1,
          ...
          },
          {
            'has_warnings' => 1,
            'end_line' => 60,
            'src' => ' for ( my $i = $ungetc_count - 1 ; $i >= 0 ; $i -- ) { my $j = $i % 256 ; is ( ord ( $fh-> getc ( ) ) , $j , "getc gets back $j" ) ; }',
            'token_num' => 41,
            'indent' => 2,
            'start_line' => 57,
            'block_id' => 5
          ...
          },
          {
            'has_warnings' => 1,
            'end_line' => 59,
            'src' => ' is ( ord ( $fh-> getc ( ) ) , $j , "getc gets back $j" ) ;',
            'token_num' => 16,
            'start_line' => 59,
            'indent' => 3,
            'block_id' => 7
